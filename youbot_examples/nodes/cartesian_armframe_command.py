...
if __name__ == "__main__":
    ...
    ctl_frame = "/arm_link_4"
    base_frame = "/arm_link_0"
    #base_frame = "/base_link"
    
    (trans,rot) = tfl.lookupTransform(base_frame, ctl_frame, rospy.Time(0))
    ...
    while (True):
        ...
        target = createPose(x, y, z, roll, pitch, yaw)
        target.header.frame_id = ctl_frame
        target.header.stamp = rospy.Time.now() - rospy.Duration(0.1)
        
        #tfl.waitForTransform(base_frame, ctl_frame,rospy.Time(),rospy.Duration(.5))
       # tfl.lookupTransform(base_frame, ctl_frame, rospy.Time.now())
        target_0 = tfl.transformPose(base_frame, target)
        ...
        succ = iks.sendPosition(target_0)
